...
rfbBool VncClient::onResize(rfbClient* cl) {
    OH_LOG_INFO(LOG_APP, "Resize: %{public}dx%{public}d", cl->width, cl->height);
...
void VncClient::onUpdate(rfbClient* cl, int x, int y, int w, int h) {
    ...
    if (count < 5 || count % 100 == 0) {
        OH_LOG_INFO(LOG_APP, "onUpdate #%{public}d: (%{public}d,%{public}d,%{public}d,%{public}d) fb=%{public}dx%{public}d",
                    count, x, y, w, h, cl->width, cl->height);
    }
    if (frameCallback_) {
        VncFrameInfo info = {
            .fbWidth = cl->width,
...
bool VncClient::connect(const char* address, int port, const char* passwd) {
    ...
    OH_LOG_INFO(LOG_APP, "Connected: %{public}dx%{public}d", client_->width, client_->height);
    return true;
}
