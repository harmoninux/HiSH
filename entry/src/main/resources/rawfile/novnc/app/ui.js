...
const UI = {
    ...
    initFullscreen() {
        // Always show the fullscreen button in ArkUI WebView environment
        // Native bridge detection happens at click time since it may not be ready at init
        // Also show if browser supports Fullscreen API
        if (!isSafari() || typeof native !== 'undefined') {
    ...
    * ------v------*/
    ...
    toggleFullscreen() {
        // Priority 1: Use Native fullscreen (ArkUI WebView environment)
        if (typeof native !== 'undefined' && native.enterFullscreen) {
            try {
                if (native.isFullscreen && native.isFullscreen()) {
                    native.exitFullscreen();
                } else {
                    native.enterFullscreen();
                }

                // Helper function to force viewport update
                const forceViewportUpdate = () => {
                    if (UI.rfb && UI.rfb._display) {
                        UI.rfb._expectedClientWidth = 0;
                        UI.rfb._expectedClientHeight = 0;
                        const w = window.innerWidth;
                        const h = window.innerHeight;
                        console.log('Autoscaling to:', w, 'x', h);
                        UI.rfb._display.autoscale(w, h);
                    }
                ...
                setTimeout(forceViewportUpdate, 1000);
                setTimeout(UI.updateFullscreenButton, 1000);
            } catch (e) {
        ...
        // Priority 2: Standard browser Fullscreen API
    ...
    updateFullscreenButton() {
        // Check native fullscreen state first
        if (typeof native !== 'undefined' && native.isFullscreen) {
            try {
